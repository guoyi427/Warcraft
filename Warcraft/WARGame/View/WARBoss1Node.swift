...
class WARBoss1Node: SKSpriteNode {
    ...
    fileprivate func _shootBullets() {
        let waitShootAction = SKAction.wait(forDuration: 0.5)
        let creatAction = SKAction.run {
            let angle = (CGFloat(M_PI) - CGFloat(M_PI)/3) / 2 + self._bulletsAnglePadding
            self._creatBullets1(origin_position: CGPoint(x: self.position.x+50, y: self.position.y - self.size.height/2), firstOriginAngle: angle)
            self._creatBullets1(origin_position: CGPoint(x: self.position.x-50, y: self.position.y - self.size.height/2), firstOriginAngle: angle)
        }
    ...
    /// 创建子弹 带移动动画
    fileprivate func _creatBullets1(origin_position: CGPoint, firstOriginAngle: CGFloat) {
        // 子弹位置
...
//        let firstOriginAngle = (CGFloat(M_PI) - sectorAngle) / 2
        ...
        for index in 0...count {
            // 实例化子弹
//            let bulletNode = WARBulletNode(type: .boss, color: SKColor.red, size: CGSize(width: 10, height: 10))
            let bulletNode = SKShapeNode(circleOfRadius: 10)
            bulletNode.fillColor = SKColor.red
